...
from jet_django.utils.db import escape_sql_identifier
...
def model_filter_class_factory(build_model, model_fields, model_relations):
    ...
    class SearchFilter(django_filters.CharFilter):
        ...
        def filter(self, qs, value):
            ...
            qs = qs.annotate(pk_as_str=Cast('pk', CharField(max_length=255))).filter(Q(pk_as_str=value) | query)
    ...
    class ModelRelationFilter(django_filters.CharFilter):
        ...
        def filter(self, qs, value):
            ...
            sql.append('SELECT {0}.{1} as id FROM {0}'.format(
                escape_sql_identifier(build_model._meta.db_table),
                escape_sql_identifier(build_model._meta.pk.name)
            ))
            ...
            for i in range(path_len):
                ...
                if not last:
                    ...
                    sql.append('JOIN {2} ON {0}.{1} = {2}.{3}'.format(
                        escape_sql_identifier(current_table._meta.db_table),
                        escape_sql_identifier(current_table_column),
                        escape_sql_identifier(related_table._meta.db_table),
                        escape_sql_identifier(related_table_column)
                    ))
                    current_table = related_table
                else:
                    current_table_column = get_field_column(current_table, item[0])
                    sql.append(' WHERE {0}.{1} IN (%s)'.format(
                        escape_sql_identifier(current_table._meta.db_table),
                        escape_sql_identifier(current_table_column)
                    ))
                    args.append(item[1])
